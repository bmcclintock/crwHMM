...
fG_fit <- foieGras::fit_ssm(rawDat,time.step=1)
fit <- fit_ssm(rawDat, time.step = 1, nbStates=1)
...
fit1 <- fit_ssm(crwHMM_data, time.step = 1, nbStates=1,verbose=2)
fit2 <- fit_ssm(crwHMM_data, time.step = 1, nbStates=2,verbose=2,parameters=list(log_beta=rep(crwOut$crwFits$`Salif Keita`$estPar[2],2),log_sigma=rep(crwOut$crwFits$`Salif Keita`$estPar[1],2)))
