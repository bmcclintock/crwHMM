...
template<class Type>
  Type objective_function<Type>::operator() ()
  {
    ...
    matrix<Type> T(4,4);
    T.setZero();
    MVNORM_t<Type> nll_proc; // Multivariate Normal for process
    
    vector<Type> allProbs(nbStates);
    Type expBetaDT;
    Type logalpha;
    Type sumalpha;
    vector<Type> lalpha(nbStates);
    ...
    for(int j = 0; j < nbStates; j++){
      ...
      // acount for initial distribution and initial j of CTCRW
      Q(0,0) = Vmu0;
      Q(1,1) = sigma2(j)*beta(j)/Type(2.0);
      Q(2,2) = Vmu0;
      Q(3,3) = Q(1,1);

      nll_proc.setSigma(Q);
      lalpha(j) = elnproduct(eln(delta(j)),-nll_proc(alpha.col(0)-alpha0.matrix()));
    }
      ...
      for(int j = 0; j < nbStates; j++){
        logalpha = LOGZERO;
        count = ccount;
        for(int k = 0; k < nbObs[t]; k++) {
          expBetaDT = exp(-beta(j)*dt(count));
          Q(0,0) = sigma2(j) * (dt(count) -(Type(2.0)/beta(j))*(Type(1.0)-expBetaDT) + (Type(1.0)/(Type(2.0)*beta(j)))*(Type(1.0)-expBetaDT * expBetaDT));
          Q(1,1) = sigma2(j) * beta(j)*(Type(1.0)-expBetaDT * expBetaDT)/Type(2.0);
          Q(0,1) = sigma2(j) * (Type(1.0) - Type(2.0)*expBetaDT + expBetaDT * expBetaDT)/Type(2.0);
          Q(1,0) = Q(0,1);
          Q(2,2) = Q(0,0);
          Q(3,3) = Q(1,1);
          Q(2,3) = Q(0,1);
          Q(3,2) = Q(0,1);
          //Q = make_Q(beta(j), sigma2(j), dt(count));
          T(0,0) = Type(1.0); 
          T(0,1) = (Type(1.0)-expBetaDT)/beta(j);
          T(1,1) = expBetaDT;
          T(2,2) = Type(1.0);
          T(2,3) = T(0,1);
          T(3,3) = T(1,1);
          //T = make_T(beta(j), dt(count));
          nll_proc.setSigma(Q);  
          allProbs(j) -= nll_proc(alpha.col(count) - T * alpha.col(count-1));
          count += 1;
        }
        for(int i=0; i < nbStates; i++){
          logalpha = elnsum(logalpha,elnproduct(lalpha(i),ltrMat(i,j)));
        }
        lnewalpha(j) = elnproduct(logalpha,allProbs(j));
        sumalpha  = elnsum(sumalpha,lnewalpha(j));
      }
      nll -= sumalpha;
      lalpha = lnewalpha - sumalpha;
    ...
    for(int i = 0; i < timeSteps; ++i) {
      ...
      if(isd(i) == 1) {
        ...
        nll +=  nll_obs(Y.col(i) - mu.col(i));   // CRW innovations
      }
    }
    ...
    //REPORT(hmmll);
    //nll += hmmll;
    //nll += forward_alg(delta0, trMat, allProbs, nbSteps);
