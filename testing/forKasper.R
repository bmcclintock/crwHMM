remotes::install_github('bmcclintock/crwHMM@kasper')
library(crwHMM)
library(SparseM)
...
# nbStates=1 (4 fixed parameters estimated)
fit1 <- crwHMM::fit_ssm(subset(rope,id=="r11"), time.step = 24, nbStates=1, verbose=2)
fit1$ssm[[1]]$tmb$env$spHess(random=TRUE) # has 0's instead of '.' in off-diagonal
...
# nbStates =2 (9 fixed parameters estimated)
...
table(as.logical(fit2$ssm[[1]]$tmb$env$spHess(random=TRUE))==0) # 2822 non-zero elements out of 156816
